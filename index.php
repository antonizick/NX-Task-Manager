...
$pageTitle = $archiveMode ? 'Nx TaskTracker — Archive' : 'Nx TaskTracker';
?>
<script>
  const ajaxUrl = "<?php echo $archiveMode ? 'archive-fetcher.php' : 'data-fetcher.php'; ?>";
</script>
<!DOCTYPE html>
<html lang="en">
<head>
...
<?php $active_page = 'wtasks'; include 'nxmenu.php'; ?>
...
<div class="nx-page">
        ...
        <table id="myTable" class="display" style="width:100%"
            ...
            <thead>
                <tr>
                    ...
                    <th>Stat</th><th>Cat</th><th>Links</th><th>&nbsp;</th>
                </tr>
                <tr>
    ...
    <div class="nx-footer">
        <form id="archiveForm" action="index.php" method="POST" style="display:inline;">
...
<div class="modal-dialog" role="document" style="max-width:860px;">
    <div class="modal-content">
        ...
        <div class="modal-body" style="display:grid;grid-template-columns:160px 1fr;gap:6px 10px;align-items:center;">
            ...
            <div><input type="number" name="priority2day" style="width:70px;" value="2"></div>

            <label>Description:</label>
            <input type="text" name="description" style="width:100%;">

            <label>Urgency:</label>
            <div><input type="number" name="urgency" style="width:70px;" value="2"></div>

            <label>Status:</label>
            <select name="status" style="width:180px;color:#fff;background-color:#047229;">
            <?php
                require_once 'dbcon.php';
                $statusResult = $conn->query("SELECT * FROM datastatus");
                while ($row = $statusResult->fetch_assoc()) {
                    $sel = ($row['pkindx'] == 3) ? ' selected' : '';
            ...
            <select name="elenav" id="categorySelect" style="width:180px;background-color:#505050;color:#fff;"
                    onchange="updateSelectStyle(this)">
            <?php
                $catResult = $conn->query("SELECT * FROM dataTaskCategories");
                echo '<option value="" data-bg="#909090" data-fc="#ffffff" selected></option>';
                while ($row = $catResult->fetch_assoc()) {
                    $bg = htmlspecialchars($row['color']);
                    $fc = htmlspecialchars($row['fcolor']);
                    echo '<option value="'.htmlspecialchars($row['indx']).'"'
                        .' data-bg="#'.$bg.'" data-fc="#'.$fc.'"'
                        .' style="background-color:#'.$bg.';color:#'.$fc.';">'
...
<form action="nxaction.php" method="post" name="actionForm" id="actionForm">
...
function updateSelectStyle(sel) {
    ...
    sel.style.backgroundColor = opt.getAttribute('data-bg') || '#505050';
    sel.style.color = opt.getAttribute('data-fc') || '#fff';
}
    ...
    const table = $('#myTable').DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                render: function(data, type, row, meta) {
                    if (data != null) {
                        const bg = row.catBGcolor ? '#'+row.catBGcolor.replace(/^#/,'') : '#fff';
                        const fc = row.catFcolor  ? '#'+row.catFcolor.replace(/^#/,'')  : '#000';
                        return `<div style="background-color:${bg};color:${fc};padding:1px 4px;border-radius:3px;">${data}</div>`;
                    }
                    const isEven = meta.row % 2 === 0;
                    const bg = isEven ? '#ffffff' : '#edf1fb';
            ...
            },
            {
                data: null,
                render: function(data, type, row) {
                    if (ajaxUrl !== 'data-fetcher.php') return '&nbsp;';
        ...
        columnDefs: [
            ...
            { width: "80px",  targets: 5, className: "text-center" },
