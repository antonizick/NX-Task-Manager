...
if (($valueAction == "recedit" || $valueAction == "recdel") && !empty($valuePass)) {
    $sql = "SELECT * FROM viewPTasks WHERE pkind = ?";
...
<form action="pnxaction.php" method="post">
    ...
    <div class="modal-dialog" role="document" style="width: 100%; max-width: 900px;">
      <div class="modal-content">
        ...
        <div class="modal-body" style="display: grid; grid-template-columns: auto 1fr; gap: 4px; width: 100%; max-width: 800px;">
          ...
          <label>status:</label>
          <select name="status" style="width: 20%; color: #ffffff; background-color: #b10202;">
            <?php
              ...
              if ($statusResult && $statusResult->num_rows > 0) {
                while ($row = $statusResult->fetch_assoc()) {
                  $selected = ($record['status'] ?? '') == $row['pkindx'] ? 'selected' : '';
                  echo '<option value="' . htmlspecialchars($row["pkindx"]) . '" style="background-color: #' . htmlspecialchars($row["color"]) . '; color: #' . htmlspecialchars($row["fcolor"]) . ';" ' . $selected . '>' . htmlspecialchars($row["status"]) . '</option>';
                }
              }
            ?>
          </select>
...
if ($valueAction == "receditact") {
    ...
    if (!empty($_POST['valpass'])) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>setTimeout(() => { window.location.href = 'ptask.php'; }, 1000);</script>";
        } else {
...
if ($valueAction == "recdelact") {
    ...
    if (!empty($valuePass)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>setTimeout(() => { window.location.href = 'ptask.php'; }, 2000);</script>";
        } else {
