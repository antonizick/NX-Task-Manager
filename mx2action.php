...
if (($valueAction == "recedit" || $valueAction == "recdel") && !empty($valuePass)) {
    $sql = "SELECT * FROM viewDataStatus WHERE indx = ?";
...
<form action="mx2action.php" method="post">
...
<div class="modal fade" id="delrecord" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    <h1 class="modal-title" id="exampleModalLabel">Remove Status</h1>
                    <!-- <button type="button" class="close" data-dismiss="modal" aria-label="Close" id="close-x"> 
                        <span aria-hidden="true">&times;</span>
                    </button> -->
                </div>
                <p><h2>You are requesting to Remove the <br><?= htmlspecialchars($record['name'] ?? '') ?> status. <br></h2></p><p>&nbsp</p>
...
if ($valueAction == "editrecordact") {
    ...
    if (!empty($_POST['valpass'])) {
        ...
        if ($stmt->execute()) {
            echo "<div class='form-container'><h2>Status Updated</h2></div>";
            echo "<script>setTimeout(() => { window.location.href = 'mx2.php'; }, 1000);</script>";
        } else {
        ...
        echo "<div class='form-container'><h2>Error</h2><p>No record ID provided.</p></div>";
    }
...
if ($valueAction == "recdelact") {
    ...
    if (!empty($valuePass)) {
        $stmt = $conn->prepare("UPDATE `datastatus` SET `rowstat`= 88888 WHERE `pkindx`= ?");
        $stmt->bind_param("s", $valuePass);
        if ($stmt->execute()) {
            echo "<div class='form-container'><h2>Status Deleted Successfully</h2></div>";
            echo "<script>setTimeout(() => { window.location.href = 'mx2.php'; }, 2000);</script>";
        } else {
